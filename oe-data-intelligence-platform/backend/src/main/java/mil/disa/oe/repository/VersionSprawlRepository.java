...
public class VersionSprawlRepository {
    ...
    public VersionSprawlRepository(NamedParameterJdbcTemplate jdbc) {
        this.jdbc = jdbc;
    }
    ...
    public VersionSprawlKpis getKpis() {
        String kpiSql = """
            SELECT
                ...
                SUM(CASE WHEN is_latest AND os_name IS NOT NULL
            ...
            FROM gold.version_sprawl_summary
        ...
        String worstSql = """
            SELECT software_name, version_count,
                   ...
                   ROUND(SUM(CASE WHEN is_latest THEN device_count ELSE 0 END)*100.0
                         / NULLIF(SUM(device_count),0), 1) AS pct_on_latest
            FROM gold.version_sprawl_summary
            WHERE snapshot_date = current_date()
            GROUP BY software_name, version_count
            ORDER BY version_count DESC LIMIT 10
            """;

        String osVerSql = """
            SELECT os_name AS name, os_version AS version,
                   device_count AS count,
                   ROUND(device_count*100.0/SUM(device_count) OVER(), 1) AS percentage,
                   is_supported
            FROM gold.version_sprawl_summary
            WHERE snapshot_date = current_date() AND software_type = 'OS'
