...
/**
 * Queries GOLD layer for Network Performance metrics.
 * Source systems: NetScout (network monitoring) + Infoblox (DNS/DHCP/IPAM)
 * Key use case: Packet loss root cause analysis
 * Tables: gold.network_performance_summary, gold.packet_loss_root_cause,
 *         gold.network_trend, gold.dns_metrics
 */
@Repository
public class NetworkPerformanceRepository {

    private static final Logger log =
        LoggerFactory.getLogger(NetworkPerformanceRepository.class);

    private final NamedParameterJdbcTemplate jdbc;

    public NetworkPerformanceRepository(NamedParameterJdbcTemplate jdbc) {
        this.jdbc = jdbc;
    }
    ...
    public NetworkKpis getKpis() {
        log.debug("Querying gold.network_performance_summary for KPIs");
        ...
        String kpiSql = """
            ...
            FROM gold.network_performance_summary
            WHERE snapshot_hour >= current_timestamp() - INTERVAL 1 HOURS
            """;

        // Packet loss root cause analysis — core OE use case
        String rootCauseSql = """
            SELECT
                segment_name, root_cause, packet_loss_rate,
                confidence_score AS confidence, affected_flows,
                recommendation,  severity
            FROM gold.packet_loss_root_cause
        ...
        String trendSql = """
            SELECT
                DATE_FORMAT(snapshot_hour, 'HH:mm') AS timestamp,
