...
public class DeviceHealthRepository {
    ...
    public DeviceHealthRepository(NamedParameterJdbcTemplate jdbc) {
        this.jdbc = jdbc;
    }
    ...
    public DeviceHealthKpis getKpis() {
        String kpiSql = """
            SELECT
                ...
                COUNT(CASE WHEN last_check_in < current_date() - 7 THEN 1 END) AS not_checked_in,
                COUNT(CASE WHEN encryption_enabled = true           THEN 1 END) AS encrypted
            FROM gold.device_health_summary
        ...
        String byTypeSql = """
            SELECT device_type, COUNT(*) AS total,
                   COUNT(CASE WHEN compliance_state = 'COMPLIANT'     THEN 1 END) AS compliant,
                   COUNT(CASE WHEN compliance_state = 'NON_COMPLIANT' THEN 1 END) AS non_compliant,
                   ROUND(COUNT(CASE WHEN compliance_state='COMPLIANT' THEN 1 END)*100.0
        ...
        String osSql = """
            SELECT os_name, os_version, COUNT(*) AS device_count,
                   is_supported, is_latest
            FROM gold.device_health_summary
            WHERE snapshot_date = current_date()
            GROUP BY os_name, os_version, is_supported, is_latest
            ORDER BY device_count DESC LIMIT 20
            """;
        ...
        BigDecimal complianceRate = total > 0
            ? new BigDecimal(compliant * 100.0 / total)
    ...
    @Retryable(retryFor = Exception.class, maxAttempts = 3,
               backoff = @Backoff(delay = 2000, multiplier = 2.0))
    public List<DeviceHealthSummary> getAll(String complianceFilter,
                                             int page, int size) {
        String sql = """
