"""
pipelines/bronze/ingest_all_sources.py
=======================================
BRONZE LAYER — Ingest all 6 raw log sources into Delta tables.

Run AFTER generate_mock_logs.py has created the log files.
Log files must exist at /home/jovyan/mock-logs/<source>/<source>_YYYYMMDD.*

Supported formats:
    Aternity      → JSONL  (app performance per session)
    NetScout      → CSV    (network flow records)
    Intune        → JSONL  (device management snapshots)
    Infoblox      → Syslog (DNS queries, RFC 5424 named format)
    Salesforce    → CSV    (incident/case tickets)
...
from pyspark.sql.functions import current_timestamp, lit, input_file_name, regexp_extract, col
...
def ingest_aternity(spark: SparkSession):
    """Aternity → s3a://bronze/raw_aternity (JSONL → Delta)"""
    ...
    df = (spark.read.schema(schema)
          .json(f"{LOG_BASE}/aternity/aternity_performance_{TODAY}.jsonl")
...
def ingest_netscout(spark: SparkSession):
    """NetScout → s3a://bronze/raw_netscout (CSV → Delta)"""
...
def ingest_intune(spark: SparkSession):
    """Intune → s3a://bronze/raw_intune (JSONL → Delta)"""
    ...
    df = (spark.read.schema(schema)
          .json(f"{LOG_BASE}/intune/intune_devices_{TODAY}.jsonl")
          .withColumn("_ingested_at", current_timestamp())
          .withColumn("_source_file", input_file_name())
          .withColumn("_batch_date",  lit(TODAY)))
...
def ingest_infoblox(spark: SparkSession):
    """Infoblox → s3a://bronze/raw_infoblox (Syslog → Delta, parsed with regex)"""
    print("\n📥 BRONZE: Infoblox (DNS Logs)...")
    raw_df = spark.read.text(f"{LOG_BASE}/infoblox/infoblox_dns_{TODAY}.log")

    # RFC 5424 named syslog pattern:
    # "May 09 12:00:00 server named[1234]: client IP#53: query: DOMAIN IN TYPE response: CODE (Xms)"
    PATTERN = (r"(\w+ \d+ \d+:\d+:\d+) (\S+) named\[\d+\]: "
               r"client (\S+)#\d+: query: (\S+) IN (\S+) "
               r"response: (\S+) \((\S+)ms\)")
...
def ingest_salesforce(spark: SparkSession):
    """Salesforce → s3a://bronze/raw_salesforce (CSV → Delta)"""
...
def ingest_sciencelogic(spark: SparkSession):
    """ScienceLogic → s3a://bronze/raw_sciencelogic (JSONL → Delta)"""
    ...
    df = (spark.read.schema(schema)
          .json(f"{LOG_BASE}/sciencelogic/sciencelogic_alerts_{TODAY}.jsonl")
          .withColumn("_ingested_at", current_timestamp())
          .withColumn("_source_file", input_file_name())
          .withColumn("_batch_date",  lit(TODAY)))
